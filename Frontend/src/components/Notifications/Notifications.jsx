...
export default function Notifications() {
    ...
    return (
        <>
            <div >
                <div  data-bs-toggle="dropdown" aria-expanded="false" onClick={() => { getNotiByUser(); resetNotis({ notis: 0 }) }}>
                ...
                <ul aria-labelledby="dropdownMenuButton1">
                    {notis.length !== 0 ?
                        notis.map((noti, id) => {
                            return (<li key={id} ><a >{noti.desc[0].toUpperCase()}{noti.desc.substr(1, noti.desc.length - 1)}</a><hr /></li>)
                        })
                        :
                        <li><a  href="#">No notis yet</a></li>
                    }
                </ul>
            </div>
        </>
